  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!ct?.includes("application/json")) {
        ...
        console.error("Non-JSON register-doctor:", { status: res.status, body: text });
        setMessage(`API error. Status: ${res.status}`);
        return;
      }
      ...
      // Token returned by backend — store it so user is logged in
