  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
        <ProfileSidebar profile={profile} role="user" activePage="profile" />
        <div className="flex-1 w-full min-w-0">
          {!profile.profile_completed && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 flex items-start gap-3 rounded-xl">
              <Info className="text-amber-500 shrink-0 mt-0.5" size={20} />
              <div><h4 className="font-semibold text-amber-800 text-sm">Profil Belum Lengkap</h4><p className="text-xs text-amber-700 mt-1">Silakan lengkapi tanggal lahir dan jenis kelamin Anda untuk dapat menggunakan fitur deteksi wajah (Scan).</p></div>
            </div>
          )}
