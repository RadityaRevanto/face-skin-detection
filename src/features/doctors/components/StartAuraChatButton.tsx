...
export function StartAuraChatButton() {
  ...
  const enterChat = async () => {
    ...
    if (!res.ok) {
      ...
      throw new Error(json?.message || "Gagal memulai chat Aura Skin");
    }
  ...
  const handleStart = async () => {
    ...
    setIsStarting(true);
    try {
      const consentRes = await fetch("/api/ai-chat/consent");
      if (!consentRes.ok) throw new Error("Gagal mengecek persetujuan AI");

      const consentJson = await consentRes.json();
      const status: ConsentStatus = consentJson.data;

      if (status.accepted) {
        await enterChat();
      } else {
        setConsent(status); // tampilkan modal consent
      }
    } catch (error: unknown) {
      customToast.error("Gagal", {
        description: (error instanceof Error ? error.message : undefined) || "Gagal memulai chat Aura Skin",
      });
    } finally {
      setIsStarting(false);
    }
  ...
  const acceptConsent = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Gagal menyimpan persetujuan AI");

      await enterChat();
    } catch (error: unknown) {
      customToast.error("Gagal", {
        description: (error instanceof Error ? error.message : undefined) || "Gagal menyimpan persetujuan AI",
      });
    } finally {
      setIsStarting(false);
      setConsent(null);
